...
- **Real OS font substitution (fontconfig on Linux)** – a `font-family`
  ...
  system running novasvg. Controlled by the `NOVASVG_USE_FONTCONFIG`
  CMake option (on by default; degrades gracefully to the previous
  behavior if fontconfig isn't found).
...
| `NOVASVG_USE_FONTCONFIG` | Use fontconfig for font-family substitution (Linux) | `ON` |
