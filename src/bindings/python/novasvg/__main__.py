...
def main():
    """
    Entry point for running 'python -m novasvg'.
    Locates the 'novasvg' CLI binary relative to this file and executes it.
    ...
    # Windows requires '.exe' extension for executables
    binary_name = "novasvg.exe" if sys.platform == "win32" else "novasvg"
    ...
    if sys.platform != "win32":
        ...
        if not (current_mode & stat.S_IXUSR):
            try:
                ...
                # Using constants from stat module instead of magic numbers
    ...
    # subprocess.run is preferred over os.execvp for better cross-platform compatibility
    try:
