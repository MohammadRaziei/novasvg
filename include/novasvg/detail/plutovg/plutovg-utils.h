...
#ifdef __cplusplus
#define PLUTOVG_ARRAY_CAST(array, value) static_cast<decltype((array).data)>(value)
#else
#define PLUTOVG_ARRAY_CAST(array, value) (value)
#endif
...
#define plutovg_array_ensure(array, count) \
    do { \
        if((array).size + (count) > (array).capacity) { \
            ...
            (array).data = PLUTOVG_ARRAY_CAST(array, realloc((array).data, newcapacity * sizeof((array).data[0]))); \
