...
#define SCALED( x )  (x)
    ...
    for ( n = 0; n < outline->n_contours; n++ )
    {
        ...
        case PVG_FT_CURVE_TAG_ON:  /* emit a single line_to */
          {
            ...
            gray_line_to(static_cast<PWorker>(user), UPSCALE(vec.x), UPSCALE(vec.y));
            continue;
          }
      ...
      gray_line_to(static_cast<PWorker>(user), UPSCALE(v_start.x), UPSCALE(v_start.y));
  ...
  PVG_FT_Raster_Render(const PVG_FT_Raster_Params *params)
  {
      ...
      while(error == ErrRaster_OutOfMemory) {
          ...
          length *= 2;
          void* heap = static_cast<void*>(malloc(length));
