...
- [x] **جایگزینیِ واقعیِ فونت از طریق fontconfig اضافه شد.** این سوالِ خودت («embedded font
      ...
      - فیکس: `FontFaceCache::getFontFaceForFamilyStack()` کل stack کاما-جدا رو یک‌جا به
        ...
        می‌شه. پشت `NOVASVG_HAVE_FONTCONFIG` قایمه (CMake خودش با `find_package(Fontconfig)`
        روی لینوکس پیدا می‌کنه، با `NOVASVG_USE_FONTCONFIG` قابل خاموش‌کردنه) — هرجا fontconfig
        لینک نشده، رفتار قبلی بدون تغییر می‌مونه.
      - یه اشتباه توی تلاش اول ارزش ثبت‌کردن داره: اول رفتم سراغ صدا زدنِ fontconfig
        **جداجدا برای هر اسم** توی stack (توقف روی اولین hit) — ولی `FcFontMatch` هیچ‌وقت
        «پیدا نشد» برنمی‌گردونه، همیشه یه چیزی می‌ده؛ پس روی همون اولین اسم («trebuchet ms»)
        با fallback عمومیِ خودِ fontconfig «موفق» می‌شد، قبل از اینکه اصلاً به «verdana» یا
        «arial» بعدی توی لیست برسه. باید کل stack مرتب رو یک‌جا بهش داد تا خودش بهترین
        alias رو انتخاب کنه.
      - تست شد در مقابل رفرنس واقعیِ Chrome: تعداد خط لیبل‌های flowchart سمپل از تقریباً
        هیچ‌کدوم-match روی ۱۰ از ۱۲ تا رسید.
      - **درباره‌ی سوال اول (`@font-face`/فونت embedded توی خودِ SVG)**: نه، ساپورت نمی‌شه —
        پارسر CSS صراحتاً هر `@`-rule رو کامل skip می‌کنه. ولی این مورد اصلاً کمکی هم نمی‌کرد
        چون خروجی خودِ مرمید هیچ فونتی embed نمی‌کنه، فقط اسم می‌ذاره.
